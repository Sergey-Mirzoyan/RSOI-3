...
type IReservationRepository interface {
	...
	GetCountReservations(username string, status string) (int, error)
...
func (rr *ReservationRepository) UpdateReservation(uid string, status string) error {
	...
	if err != nil {
		fmt.Printf("Failed to update with uid %s in db\n", uid)
		return err
	}
	if affected == 0 {
		err = errors.New("No records were founded for the given username")
	}
	return err
}
...
func (rr *ReservationRepository) GetReservation(ruid string) (*models.Reservation, error) {
	...
	if err != nil {
		fmt.Printf("Failed to get from data base\n")
		return nil, err
	}
	res := &models.Reservation{
		Id:             BytesToInt(data[0][0]),
		ReservationUid: BytesToUid(data[0][1]),
		Username:       BytesToString(data[0][2]),
		BookUid:        BytesToUid(data[0][3]),
		LibraryUid:     BytesToUid(data[0][4]),
		Status:         BytesToString(data[0][5]),
		StartDate:      BytesToString(data[0][6]),
		TillDate:       BytesToString(data[0][7]),
	}

	return res, err
}

func (rr *ReservationRepository) GetCountReservations(username string, status string) (int, error) {
	res := -1
	if status == "" {
		status = "%"
	}
...
func (rr *ReservationRepository) GetUserReservations(username string) ([]*models.Reservation, error) {
	...
	for _, row := range data {
		res = append(res, &models.Reservation{
			Id:             BytesToInt(row[0]),
			ReservationUid: BytesToUid(row[1]),
			Username:       BytesToString(row[2]),
			BookUid:        BytesToUid(row[3]),
			LibraryUid:     BytesToUid(row[4]),
			Status:         BytesToString(row[5]),
			StartDate:      BytesToString(row[6]),
			TillDate:       BytesToString(row[7]),
		})
	}
